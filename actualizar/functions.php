...
function do_connect($db) {
   ...
   if (empty($db_link)) {
      ...
      if(!@mysql_select_db($db['database'], $db_link)) return -1;
      else return 1;
	}
}
/**
...
function do_query($query, $do = TRUE, $status = NULL) {
   ...
   if ($do) {
      ...
      if (is_object($query)) return do_object_query($query);
...
function generate_collection($collection) {
   ...
   foreach($collection as $key => $element) {
      $list[] = "<dl><dt class=\"cf\"><label for=\"$key\">{$element->name}:</label><br /></dt><dd class=\"cf\"><div class=\"custom-control custom-checkbox\"><input type=\"checkbox\" class=\"custom-control-input\" value=\"1\" name=\"update[$key]\" id=\"$key\" checked=\"checked\"><label class=\"custom-control-label\" for=\"$key\"></label></div></dd></dl>";
   }
   return implode("\n", $list);
}
...
function check_install_step($check_collection, $key) {
   ...
   if ($data[0] == 'table' || $data[0] == 'table_not') {
      ...
      foreach($data[2] as $kk => $f) {
         switch($f[1]) {
            case 'DROP':
               if (!isset($list[$f[0]])) $c[$kk] = FALSE;
            break;
            case 'ADD':
               if (isset($list[$f[0]])) $c[$kk] = FALSE;
            break;
            case 'CHANGE':
               if (!isset($list[$f[0]]) && isset($list[$f[2]])) $c[$kk] = FALSE;
            break;
            default:
               $c[$kk] = FALSE;
         }
...
function resultado_html($collection, $resultado) {
   ...
   foreach($collection as $key => $element) {
      $rst = $resultado[$key][0];
      $html_rst = ($rst == QUERY_RESULT_SKIP ? 'Omitido' : ($rst == QUERY_RESULT_OK ? 'OK' : 'Error'));
      $html_class = ($rst == QUERY_RESULT_SKIP ? 'warning' : ($rst == QUERY_RESULT_OK ? 'success' : 'danger'));
      $error_html = $resultado[$key][0] == QUERY_RESULT_ERROR ? "<div class=\"alert alert-danger\">{$resultado[$key][1]}</div>" : '';
      $queries_count = count($element->queries);
      $queries_html = $queries_count.' consulta'.($queries_count > 1 ? 's' : '');
      $list[] = "<dl><dt class=\"cf\"><label for=\"$key\">{$element->name}:<span>$queries_html</span></label></dt><dd class=\"cf\"><div class=\"badge badge-$html_class\">$html_rst</div></dd></dl>$error_html";
   }
   return implode("\n", $list);
}
