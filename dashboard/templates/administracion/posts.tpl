			...
			<tbody>
				...
				<tr id="post_{$p.post_id}">
					...
					<td class="admin_actions">
						<a href="{$tsConfig.url}/posts/editar/{$p.post_id}" title="Editar Post"><i class="fas fa-edit"></i></a>
						{if $p.post_status == 2}
						<a href="#" onclick="admin.posts.borrar({$p.post_id}); return false" title="Borrar Post permanentemente"><i class="far fa-trash"></i></a>
						{else}
						<a href="#" onclick="mod.posts.borrar({$p.post_id}, 'posts', null); return false;" title="Borrar Post"><i class="fas fa-times"></i></a>
						{/if}
					</td>
				</tr>
				{/foreach}
