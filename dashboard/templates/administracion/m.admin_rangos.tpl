...
<div id="res" class="boxy-content" style="position:relative">
         ...
         <tbody>
            ...
            <tr>
               ...
               <td class="admin_actions">
                  <a href="?act=editar&rid={$r.id}&t=s" title="Editar Rango"><i class="fas fa-edit"></i></a>
                  {if $r.id > 3}<a href="?act=borrar&rid={$r.id}" title="Borrar Rango"><i class="fas fa-times"></i></a>{/if}
                  {if $tsConfig.c_reg_rango == $r.id}<a title="Rango Predeterminado al registro"><i class="fas fa-thumbtack text-info"></i></a>{else}<a id="f_2" href="#" onclick="location.href = '{$tsConfig.url}/admin/rangos/?act=setdefault&rid={$r.id}'; $('#f_2').style.cursor=wait"><i style="cursor:pointer;" class="fas fa-map-pin text-success" title="Establecer Predeterminado"></i></td>{/if}
               </td>
            </tr>
            {/foreach}
         </tbody>
         <tfoot>
         <td colspan="7" style="text-align:right"><a class="btn btn-success" href="?act=nuevo&t=s">Agregar nuevo rango &raquo;</a></td>
         </tfoot>
      </table>
   </div>
   </div>
      ...
      <table class="table table-data2">
         ...
         <tbody>
            ...
            <tr>
               ...
               <td><img src="{$tsConfig.images}/icons/ran/{$r.imagen}" /></td>
               <td class="admin_actions">
                  <a href="?act=editar&rid={$r.id}&t=p" title="Editar Rango"><i class="fas fa-edit"></i></a>
                  {if $r.id > 3}<a href="?act=borrar&rid={$r.id}" title="Borrar Rango"><i class="fas fa-times"></i></a>{/if}                  
               </td>
            </tr>
            {/foreach}
         ...
         <tfoot>
         <td colspan="9" style="text-align:right"><a class="btn btn-success" href="?act=nuevo">Agregar nuevo rango &raquo;</a></td>
         </tfoot>
      </table>
   </div>
   </div>
   ...
   <form action="" method="post">
      <fieldset>
         <div class="btn-group btn-block mb-4">
            <div id="b1" class="btn btn-success">Básico</div>
            <div id="b2" class="btn btn-warning">Permisos</div>
         </div>         
         <script>
            $('#b1').click(function(){
               $('#tab1').fadeIn('slow'), $('#tab2').hide('slow');
               $('#b2').addClass('btn-warning').removeClass('btn-success');
               $('#b1').addClass('btn-success').removeClass('btn-warning');
            });
            $('#b2').click(function(){
               $('#tab2').fadeIn('slow'), $('#tab1').hide('slow');
               $('#b1').addClass('btn-warning').removeClass('btn-success');
               $('#b2').addClass('btn-success').removeClass('btn-warning');
            });
         </script>
         <div id="tab1">
            <dl>
               <dt><label for="rName">T&iacute;tulo:</label></dt>
               <dd><input type="text" id="rName" class="form-control" name="rName" value="{$tsRango.r_name}"/></dd>
            </dl>
            <dl>
               ...
               <dd><input type="text" id="rColor" class="jscolor form-control" name="rColor" value="{$tsRango.r_color}" style="color:#{$tsRango.r_color}; font-weight:bold;"/></dd>
            </dl>
            <dl>
               ...
               <dd><input type="text" id="gopfd" class="form-control" name="global-pointsforday" value="{$tsRango.permisos.gopfd}"/></dd>
            </dl>
            <dl>
               <dt><label for="gopfp">Puntos por post</label><span>Puntos que puede dar en cada post.</span></dt>
               <dd><input type="text" id="gopfp" class="form-control" name="global-pointsforposts" value="{$tsRango.permisos.gopfp}"/></dd>
            </dl>
            <dl>
               <dt><label for="goaf">Anti-flood</label><span>Tiempo que deben esperar entre acci&oacute;n.</span></dt>
               <dd><input type="text" id="goaf" class="form-control" name="global-antiflood" value="{$tsRango.permisos.goaf}"/></dd>
            </dl>
            <dl>               
               ...
               <label class="mr-3" onclick="$('#gocpr').slideDown();"><input name="global-type" type="radio" id="ai_type" value="1" {if $tsRango.r_type == 1}checked{/if} class="radio"/><span title="Del usuario">Puntos</span></label>
               
               <label class="mr-3" onclick="$('#gocpr').slideDown();"><input name="global-type" type="radio" id="ay_type" value="2" {if $tsRango.r_type == 2}checked{/if} class="radio"/>Posts</label>
               
               <label class="mr-3" onclick="$('#gocpr').slideDown();"><input name="global-type" type="radio" id="ay_type" value="3" {if $tsRango.r_type == 3}checked{/if} class="radio"/>Fotos</label>
               
               <label class="mr-3" onclick="$('#gocpr').slideDown();"><input name="global-type" type="radio" id="ay_type" value="4" {if $tsRango.r_type == 4}checked{/if} class="radio"/><span title="De posts y fotos">Comentarios</span></label>
               
               <label class="mr-3" onclick="$('#gocpr').slideUp();"><input name="global-type" type="radio" id="ay_type" value="0" {if $tsRango.r_type == 0}checked{/if} class="radio"/>Especial</label>
               </dd>
            ...
            <dl>
               ...
               <select name="r_img" class="form-control d-inline" id="cat_img" style="width:164px">
            ...
            <hr />
            <input type="button" onclick="$('#tab2').show('slow'); $('#tab1').hide('slow'); $('#b2').css('border','1px solid #4D90FE'); $('#b1').css('border','0px');" value="Continuar" class="btn btn-success"/>
         </div>
   ...
   <form action="" method="post" id="admin_form">
      <div class="alert alert-danger">Si borras este rango todos los usuarios que est&eacute;n en &eacute;l, ser&aacute;n asignados al rango
         ...
         <select name="new_rango" class="form-control">{foreach from=$tsRangos item=r}<option value="{$r.rango_id}" style="color:#{$r.r_color}; padding:2px 20px 0;" {if $r.rango_id == 3}selected{/if}>{$r.r_name}</option>{/foreach}</select> <br /> &iquest;Realmente deseas borrar este rango?</div>
         
         <label>&nbsp;</label> <input type="submit" name="save" value="S&iacute;, Continuar &raquo;" class="btn btn-success">
      </form>
      {/if}
   </div>