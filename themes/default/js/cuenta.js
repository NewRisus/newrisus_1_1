...
var cuenta = {
   ...
   alert: function(secc, title, body) {
      $('div.alert-cuenta.cuenta-' + secc).html('<h6>' + title + '</h6>');
   ...
   chgpais: function() {
      ...
      var el_estado = $('select[name=estado]');
      console.log(pais);

      //No se selecciono ningun pais.
      if (empty(pais)) {
         $('select[name=estado]').addClass('disabled').attr('disabled', 'disabled').val('');
      } else {
         //Obtengo las estados
         $(el_estado).html('');
         $('#loading').fadeIn(250);
      ...
      $.ajax({
         ...
         success: function(r) {
            ...
            if (r.error) {
               ...
               cuenta.alert(secc, '<div class="alert alert-success" role="alert">Los cambios fueron aceptados y ser&aacute;n aplicados.</div>');
               if (r.porc != null) {
...
/* COMPLEMENTO AGREGADO PARA HACER MEJOR
 EL INPUT FILE */
'use strict';
$(function() {
