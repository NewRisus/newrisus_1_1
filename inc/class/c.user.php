...
class tsUser {
	...
	*/
	function loadUser($login = FALSE)
    {
		...
		$rango = db_exec(array(__FILE__, __LINE__), 'query', 'SELECT u.user_id, u.user_rango, r.* FROM u_miembros AS u LEFT JOIN u_rangos AS r ON u.user_rango = r.rango_id WHERE u.user_id = '.$this->info['user_id'].' LIMIT 1');
        $this->info['DataUser'] = db_exec('fetch_assoc', $rango);
		...
		$this->rango = $this->info['DataUser']['r_name'];
		$this->r_color = $this->info['DataUser']['r_color'];
	...
	*/
	function loginUser($username, $password, $remember = FALSE, $redirectTo = NULL){
      ...
      if (filter_var($username, FILTER_VALIDATE_EMAIL)) {
		...
		/* CONSULTA */
      $query = db_exec(array(__FILE__, __LINE__), 'query', 'SELECT user_id, user_name, user_password, user_activo, user_baneado FROM u_miembros WHERE LOWER('. ( $email ? 'user_email' : 'user_name' ) .') = \''.$username.'\' LIMIT 1');
        //
