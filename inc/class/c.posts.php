  ...
  */
  ...
  function newPost() {
    ...
    if ($tsUser->is_admod || $tsUser->permisos['gopp']) {
      //
      $postData = array('date' => time(), 'title' => $tsCore->parseBadWords($tsCore->setSecure($_POST['titulo'], true)), 2, 'body' => $tsCore->setSecure($_POST['cuerpo']), 'tags' => $tsCore->parseBadWords($tsCore->setSecure($_POST['tags'], true)), true, 1, 'category' => intval($_POST['categoria']),);
      //ANTIFLOOD
      $antiflood = 2;
  ...
  function savePost() {
    ...
    //
    if ($data['post_status'] != '0' && !$tsUser->is_admod && !$tsUser->permisos['moedpo']) return 'El post no puede ser editado.';
    //
    $postData = array('title' => $tsCore->parseBadWords($_POST['titulo'], true), 'body' => $tsCore->setSecure($_POST['cuerpo'], true), 'tags' => $tsCore->parseBadWords($tsCore->setSecure($_POST['tags'], true)), 'category' => $_POST['categoria'],);
    // VACIOS
    foreach ($postData as $key => $val) {
      $val = trim(preg_replace('/[^ A-Za-z0-9]/', '', $val));
      $val = str_replace(' ', '', $val);
  ...
  /* setNP() :: POST ANTERIOR, SIGUIENTE O ALEATORIO */
    function setNP() {
        global $tsUser, $tsCore;
        $action = $_GET['action'];
        if ($action == 'fortuitae') {
    ...
    */
    function getCatData() {
        ...
        $cat = intval($_GET['cat']);
        //
        $query = db_exec(array(__FILE__, __LINE__), 'query', 'SELECT c_nombre, c_seo, c_fa, c_color, c_desc FROM p_categorias WHERE c_seo = \'' . $tsCore->setSecure($_GET['cat']) . '\' LIMIT 1');
        $data = db_exec('fetch_assoc', $query);
        return $data;
    }
    # ========== MOD FORO ========== #
    ...
    */
    function getLastPosts($category = NULL, $subcateg = NULL, $sticky = false) {
        ...
        /*********/
        $query = db_exec(array(__FILE__, __LINE__), 'query', 'SELECT p.post_id, p.post_user, p.post_category, p.post_title, p.post_date, p.post_comments, p.post_body, p.post_puntos, p.post_private, p.post_sponsored, p.post_status, p.post_sticky, u.user_id, u.user_name, u.user_activo, u.user_baneado, c.c_nombre, c.c_seo, c.c_fa, c.c_color FROM p_posts AS p LEFT JOIN u_miembros AS u ON p.post_user = u.user_id  ' . ($tsUser->is_admod && $tsCore->settings['c_see_mod'] == 1 ? '' : ' && u.user_activo = \'1\' && u.user_baneado = \'0\'') . ' LEFT JOIN p_categorias AS c ON c.cid = p.post_category WHERE ' . ($tsUser->is_admod && $tsCore->settings['c_see_mod'] == 1 ? 'p.post_id > 0' : 'p.post_status = \'0\' && u.user_activo = \'1\' && u.user_baneado = \'0\'') . '  ' . $c_where . ' ' . $s_where . ' GROUP BY p.post_id ORDER BY ' . $s_order . ' DESC LIMIT ' . $start);
        $elements = array("/(?i)\[url\=(http|https|ftp|irc|ed2k|gopher|telnet|gopher|telnet)(\:\/\/)([^\<\>[:space:]]+?)\](.+?)(\[\/url\])/i", "/(?i)\[quote\=([^\n\r\t\<\>]+?)\]([^\a]+?)\[\/quote\]/i", "/(?i)\[img\=([0-9]{1,3})x([0-9]{1,3})\](http|https|ftp|irc|ed2k|gopher|telnet)(\:\/\/)([^\<\>[:space:]]+?)(\[\/img\])/i", "/(?i)\[img\=(http|https|ftp|irc|ed2k|gopher|telnet)?(\:\/\/)?([^\<\>[:space:]]+)\]/i", "/(?i)\[swf\=(.+?)\]/i", "/(?i)\[goear\=(.+?)\]/i", "/(?i)\[goear\]([a-z0-9]{7})\[\/goear\]/i");
        $size = count($elements);
    ...
    */
    function getPost() {
        ...
        // CATEGORIAS
        $query = db_exec(array(__FILE__, __LINE__), 'query', 'SELECT c.c_nombre, c.c_fa, c.c_color, c.c_seo FROM p_categorias AS c  WHERE c.cid = \'' . $postData['post_category'] . '\'');
        ...
        // FECHA
        //$postData['post_date'] = strftime("%d de %m del %Y",$postData['post_date']);
        // YA LO VOTE?
        ...
        if (!empty($data['post_id']) || !empty($tsUser->is_admod)) {
            ...
            if (db_exec(array(__FILE__, __LINE__), 'query', 'DELETE FROM p_posts WHERE post_id = \'' . (int)$post_id . '\'')) {
                if (db_exec(array(__FILE__, __LINE__), 'query', 'DELETE FROM p_comentarios WHERE c_post_id = \'' . (int)$post_id . '\'')) {
                    if (db_exec(array(__FILE__, __LINE__), 'query', 'INSERT INTO `p_borradores` (b_user, b_date, b_title, b_body, b_tags, b_category, b_status, b_causa) VALUES (\'' . $tsUser->uid . '\', \'' . time() . '\', \'' . $tsCore->setSecure($data['post_title']) . '\', \'' . $tsCore->setSecure($data['post_body']) . '\', \'\', \'' . $data['post_category'] . '\', \'2\', \'\')')) return "1: El post fue eliminado satisfactoriamente.";
                }
            } else {
    ...
    */
    function getRelated($tags) {
        ...
        //
        $query = db_exec(array(__FILE__, __LINE__), 'query', "SELECT DISTINCT p.post_id, p.post_title, p.post_category, p.post_private, c.c_seo, c.c_fa, c.c_color FROM p_posts AS p LEFT JOIN p_categorias AS c ON c.cid = p.post_category WHERE MATCH (post_tags) AGAINST ('$tags' IN BOOLEAN MODE) AND p.post_status = 0 AND post_sticky = 0 ORDER BY rand() LIMIT 0,10");
        //
        $data = result_array($query);
        //
        return $data;
    ...
    */
    function getLastComentarios() {
        global $tsUser, $tsCore;
        //
        $query = db_exec(array(__FILE__, __LINE__), 'query', 'SELECT cm.cid, cm.c_status, u.user_name, u.user_activo, u.user_id, u.user_rango, r.rango_id, r.r_color, u.user_baneado, p.post_id, p.post_title, p.post_status, c.c_seo FROM p_comentarios AS cm LEFT JOIN u_miembros AS u ON cm.c_user = u.user_id LEFT JOIN u_rangos AS r ON r.rango_id = u.user_rango LEFT JOIN p_posts AS p ON p.post_id = cm.c_post_id LEFT JOIN p_categorias AS c ON c.cid = p.post_category ' . ($tsUser->is_admod && $tsCore->settings['c_see_mod'] == 1 ? '' : 'WHERE p.post_status = \'0\'  AND cm.c_status = \'0\' AND u.user_activo = \'1\' && u.user_baneado = \'0\'') . ' ORDER BY cid DESC LIMIT 10');
    ...
    */
    function getLastUser() {
        global $tsCore;
        //
        $query = db_exec(array(__FILE__, __LINE__), 'query', 'SELECT * FROM u_miembros WHERE user_activo= \'1\' AND user_baneado= \'0\' ORDER BY user_id DESC LIMIT 1'); /* LIMIT 14 ? le das el numero que quieras */
    ...
    */
    function getComentarios($post_id) {
        ...
        $query = db_exec(array(__FILE__, __LINE__), 'query', 'SELECT u.user_name, u.user_seguidores, u.user_posts, u.user_puntos, u.user_comentarios, u.user_activo, u.user_baneado, u.user_rango, r.rango_id, r.r_name, r.r_color, c.mejor_comentario, p.user_pais, p.user_sexo, p.user_firma, c.* FROM u_miembros AS u LEFT JOIN p_comentarios AS c ON u.user_id = c.c_user LEFT JOIN u_rangos AS r ON r.rango_id = u.user_rango LEFT JOIN u_perfil AS p ON u.user_id = p.user_id WHERE c.c_post_id = \'' . (int)$post_id . '\' ' . ($tsUser->is_admod ? '' : 'AND c.c_status = \'0\' AND u.user_activo = \'1\' && u.user_baneado = \'0\'') . ' ORDER BY c.cid DESC LIMIT ' . $start);
        // STATUS
        $is_online = (time() - ($tsCore->settings['c_last_active'] * 60));
        $is_inactive = (time() - (($tsCore->settings['c_last_active'] * 60) * 2)); // DOBLE DEL ONLINE
        if ($data['user_lastactive'] > $is_online) $data['status'] = array('t' => 'Usuario Online', 'css' => 'online');
    ...
    */
    function editComentario() {
        ...
        if ($tsUser->is_admod || ($tsUser->uid == $cuser['c_user'] && $tsUser->permisos['goepc']) || $tsUser->permisos['moedcopo']) {
            // ANTI FLOOD
            $tsCore->antiFlood();
            if (db_exec(array(__FILE__, __LINE__), 'query', 'UPDATE p_comentarios SET c_body = \'' . $comentario . '\' WHERE cid = \'' . (int)$cid . '\'')) return '1: El comentario fue editado.';
    ...
    */
    function delComentario() {
        ...
        if (!db_exec('num_rows', db_exec(array(__FILE__, __LINE__), 'query', 'SELECT cid FROM p_comentarios WHERE cid = \'' . (int)$comid . '\''))) {
            return '0: El comentario no existe';
        } // [17/04/2012] Evitar miles de comentarios
    ...
    function votarPost() {
        ...
        if ($tsUser->is_admod || $tsUser->permisos['godp']) {
            ...
            if (!ctype_digit($_POST['puntos'])) {
                return '0: S&oacute;lo puedes votar con n&uacute;meros.';
            }
    ...
    */
    function getFavoritos() {
        global $tsCore, $tsUser;
        //
        $query = db_exec(array(__FILE__, __LINE__), 'query', 'SELECT f.fav_id, f.fav_date, p.post_id, p.post_title, p.post_date, p.post_puntos, COUNT(p_c.c_post_id) as post_comments,  c.c_nombre, c.c_seo, c.c_fa, c_color FROM p_favoritos AS f LEFT JOIN p_posts AS p ON p.post_id = f.fav_post_id LEFT JOIN p_categorias AS c ON c.cid = p.post_category LEFT JOIN p_comentarios AS p_c ON p.post_id = p_c.c_post_id && p_c.c_status = \'0\' WHERE f.fav_user = \'' . $tsUser->uid . '\' AND p.post_status = \'0\' GROUP BY c_post_id');
        $data = result_array($query);
        //
        foreach ($data as $fav) {
    ...
    */
    function getQuery() {
        ...
        //
        $query = db_exec(array(__FILE__, __LINE__), 'query', 'SELECT p.post_id, p.post_user, p.post_category, p.post_title, p.post_date, p.post_comments, p.post_favoritos, p.post_puntos, u.user_name, c.c_seo, c.c_nombre, c.c_fa, c.c_color FROM p_posts AS p LEFT JOIN u_miembros AS u ON u.user_id = p.post_user LEFT JOIN p_categorias AS c ON c.cid = p.post_category WHERE p.post_status = \'0\' ' . $where_cat . ' ' . $w_autor . ' ' . $w_search . ' ORDER BY p.post_date DESC LIMIT ' . $data['pages']['limit']);
