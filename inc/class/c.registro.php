...
class tsRegistro {
	...
	public function checkUserEmail() {
		...
		if (!empty($username) || !empty($email)) {
			...
			if (db_exec('num_rows', $query) > 0) {
				$valid = '0: El ' . $which . ' ya se encuentra registrado.';
			}
			// EXISTE
			if (db_exec('num_rows', db_exec(array(__FILE__, __LINE__), 'query', 'SELECT id FROM w_blacklist WHERE (type = \'3\' && value = \'' . $tsCore->setSecure(strstr($email, '@')) . '\') ||  (type = \'4\' && value = \'' . $tsCore->setSecure(strstr($email, '@', true)) . '\') || (type = \'4\' && value = \'' . $tsCore->setSecure($username) . '\') LIMIT 1'))) {
	...
	function registerUser() {
		...
		/** reCAPTCHA **/
		if($tsCore->settings['c_recaptcha'] == 1) {
		...
		if (db_exec('num_rows', $query) > 0 || !filter_var($tsData['user_email'], FILTER_VALIDATE_EMAIL) || $tsCore->settings['c_reg_active'] == 0) {
		...
		if (db_exec(array(__FILE__, __LINE__), 'query', 'INSERT INTO `u_miembros` (`user_name`, `user_password`, `user_email`, `user_rango`, `user_registro`) VALUES (\'' . $tsCore->setSecure($tsData['user_nick']) . '\', \'' . $tsCore->setSecure($key) . '\', \'' . $tsCore->setSecure($tsData['user_email']) . '\', ' . (empty($tsCore->settings['c_reg_rango']) ? 3 : $tsCore->settings['c_reg_rango']) . ', \'' . $tsData['user_registro'] . '\')')) {
			...
			db_exec(array(__FILE__, __LINE__), 'query', 'INSERT INTO `u_perfil` (`user_id`, `user_dia`, `user_mes`, `user_ano`, `user_sexo`) VALUES (\'' . (int) $tsData['user_id'] . '\', \''.(int)date('d').'\', \''.(int)date('m').'\', \''.(int)date('Y').'\', \''.(int)$tsData['user_sexo'].'\')');
         ...
         $numero = rand(1,3);
			...
			if (empty($tsCore->settings['c_reg_activate'])) {
				...
				<div style="background:#FFF;padding:10px;font-size:14px">
					...
					return '1: <div class="alert alert-danger">Te hemos enviado un correo a <b>' . $to . '</b> con los &uacute;ltimos pasos para finalizar con el registro.<br><br>Si en los pr&oacute;ximos minutos no lo encuentras en tu bandeja de entrada, por favor, revisa tu carpeta de correo no deseado, es posible que se haya filtrado.<br><br>&iexcl;Muchas gracias!</div>';
				} else {
					return '0: <div class="alert alert-warning">Ocurri&oacute; un error, int&eacute;ntelo de nuevo.</div>';
				}
			} else {
