...
public function setNotificacion($type = NULL, $user_id = NULL, $obj_user = NULL, $obj_uno = 0, $obj_dos = 0, $obj_tres = 0){
    ...
    */
     ...
     */
    private function allowNotifi($tipo, $usuario) {
        # CONSULTAMOS
        $query = db_exec(array(__FILE__, __LINE__), 'query', "SELECT c_monitor FROM u_portal WHERE user_id = {$usuario} LIMIT 1");
        $data = db_exec('fetch_assoc', $query);
        ...
        # VERIFICAMOS
        if(in_array($tipo, $filtros)) return true;
        else return false;
    }
}