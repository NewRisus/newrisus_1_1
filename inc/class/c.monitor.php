    ...
    */
    ...
    private function makeMonitor(){
        $this->monitor = array(
            ...
            3 => array('text' => 'vot&oacute; _REP_ tu', 'ln_text' => 'post', 'css' => 'voto_'),
...
public function setNotificacion($type = NULL, $user_id = NULL, $obj_user = NULL, $obj_uno = 0, $obj_dos = 0, $obj_tres = 0){
    ...
    */
     ...
     */
    public function setMuroRepost($pub_id = NULL, $p_user = NULL, $p_user_pub = NULL){
       global $tsUser;
        //
    ...
    public function getNotificaciones($unread = false){
        ...
        // SI VA AL MONITOR ENTONCES ACTUALIZAMOS PARA QUE YA NO SE VEAN EN EL MENUBAR
            ...
            for($i > 0; $i < 18; $i++) {
                $filtro[$i] = $this->allowNotifi($i, $tsUser->uid);
            }
    ...
    */
    function makeConsulta($data = NULL){
    ...
    */
    private function makeOracion($data = NULL){
        ...
        switch($no_type){
            ...
            break;
            case 15:
                $oracion['text'] = 'Recibiste una nueva <span title="'.$data['m_title'].'"><b>medalla</b> <img src="'.$site_url.'/themes/'.TS_TEMA.'/images/icons/med/'.$data['m_image'].'_16.png"/></span>';
            break;
            case 16:
    ...
    */
    public function getFollows($type = NULL, $user_id = 0){
        global $tsCore, $tsUser;
        // VARS
    ...
    */
    ...
    public function setFiltro(){
        ...
        $filtros = $_POST['fid'];
        //
        if(db_exec(array(__FILE__, __LINE__), 'query', "UPDATE u_portal SET c_monitor = {$filtros} WHERE user_id = {$tsUser->uid}")) return '1: Configuraciones guardadas.';
    ...
    private function allowNotifi($type, $user_id){
        ...
        # HACK
        if(empty($monitor)) return false;
        # PROCESAMOS
        $filtros = explode(',', $monitor);
        # VERIFICAMOS
