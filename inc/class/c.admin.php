...
class tsAdmin {
    ...
    */
    function newTema() {
        ...
        if (db_exec(array(__FILE__, __LINE__), 'query', 'INSERT INTO `w_temas` (`t_name`, `t_url`, `t_path`, `t_copy`) VALUES (\'' .
            $tsCore->setSecure($temadb['nombre']) . '\', \'' . $tsCore->setSecure($temadb['url']) . '\', \'' . $tsCore->setSecure($tema_path) . '\', \'' . $tsCore->setSecure($temadb['copy']) . '\')'))
            return 1;
        else
    ...
    */
...

    {
        ...
        $query = db_exec(array(__FILE__, __LINE__), 'query', 'SELECT * FROM p_categorias WHERE cid = \'' . (int)$cid . '\' LIMIT 1');
    ...
    */
...

    {
        ...
        $hcat = $tsCore->setSecure($_POST['hcat']);
        $cfa = $tsCore->setSecure($tsCore->parseBadWords($_POST['c_fa']));
        $ccolor = $tsCore->setSecure($_POST['c_color']);
        $cdesc = $tsCore->setSecure($tsCore->parseBadWords($_POST['c_desc']));

        if (db_exec(array(__FILE__, __LINE__), 'query', 'UPDATE `p_categorias` SET c_nombre = \'' . $tsCore->setSecure($c_nombre) . '\', c_hcat = \'' . $tsCore->setSecure($hcat) . '\', c_seo = \'' . $tsCore->setSecure($tsCore->setSEO($c_nombre, true)) . '\', c_fa = \'' . $tsCore->setSecure($cfa) . '\', c_color = \'' . $tsCore->setSecure($ccolor) . '\',c_desc = \'' . $tsCore->setSecure($cdesc) . '\' WHERE cid = \'' . (int)$cid . '\''))
    ...
    */
...

    {
        ...
        if (db_exec(array(__FILE__, __LINE__), 'query', 'INSERT INTO `p_categorias` (`c_orden`, `c_nombre`, `c_seo`, `c_fa`, `c_color`, `c_desc`) VALUES (\'' .

            $orden . '\', \'' . $c_nombre . '\',\'' . $tsCore->setSEO($c_nombre, true) . '\', \'' . $cfa . '\', \'' . $ccolor . '\', \'' . $cdesc . '\')'))
