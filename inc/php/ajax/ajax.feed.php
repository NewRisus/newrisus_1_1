...
\*********************************/
	...
	switch($action){
		...
		case 'feed-themes':
			//<--- CONSULTAR ACTUALIZACIONES OFICIALES Y VERIFICAR VERSIÓN ACTUAL DE ESTE SCRIPT
            $json = $tsCore->getUrlContent('https://newrisus.com/feed/newrisus.json');
            echo $json;
			//--->
		break;
		case 'feed-version':
             ...
             */
            $version_now = 'New Risus 1.1';
            # ACTUALIZAR VERSIÓN
            if($tsCore->settings['version'] != $version_now){
			    db_exec(array(__FILE__, __LINE__), 'query', 'UPDATE `w_configuracion` SET version = \''.$version_now.'\', version_code = \'new_risus_1_1\' WHERE tscript_id = \'1\' LIMIT 1');
