...
\*********************************/
   ...
   // LISTA NEGRA
	...
	/** Descargas **/
   } elseif($action == 'descargas'){
		include("../class/c.descargas.php");
		$tsDescargas = new tsDescargas();
		$id = (int)$_GET['id'];
      if(!($act)) {
      	$smarty->assign("tsDescargas",$tsDescargas->getDescargas());
      } elseif($act == 'nueva'){
			if(!empty($_POST['d_title'])){
         	if($tsDescargas->newDescarga()) $tsCore->redirectTo($tsCore->settings['url'].'/admin/descargas?save=true');
         }
     	} elseif($act == 'editar'){
     		if($_POST['edit']){
				if($tsDescargas->GuardarDescarga($id)) $tsCore->redirectTo($tsCore->settings['url'].'/admin/descargas?save=true');
			} else {
				$smarty->assign("tsDescargas",$tsDescargas->getDescarga($id));
		
			}
      }  elseif($act == 'borrar'){
         if($tsDescargas->delDescarga($id)) $tsCore->redirectTo($tsCore->settings['url'].'/admin/descargas?borrar=true');
      }
   /** MEDALLAS **/
